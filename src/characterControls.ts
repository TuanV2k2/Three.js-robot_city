...
import { A, ACTIONS, A_1, A_2, A_3, A_4, D, DIRECTIONS, S, W } from './utils'
...
export class CharacterControls {
    ...
    public update(delta: number, keysPressed: any) {
        var play = '';
        const actionsPressed = ACTIONS.some(key => keysPressed[key] == true)
        if (actionsPressed){ 
                if (keysPressed[A_1]){
                    play = "Jump"
                }else if (keysPressed[A_2]){
                    play = "Dance"
                }else if (keysPressed[A_3]){
                    play = "Wave"
                }else if (keysPressed[A_4]){
                    play = "ThumbsUp"
                }
